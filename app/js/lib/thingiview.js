...
Thingiview.prototype.initScene = function ()
{
    ...
    this.alertBox.style.top = '25%';
    this.alertBox.style.left = '25%';
    this.alertBox.style.width = '50%';
    this.alertBox.style.height = '50%';
    this.alertBox.style.backgroundColor = '#dddddd';
    this.alertBox.style.padding = '10px';
    // alertBox.style.overflowY = 'scroll';
    this.alertBox.style.display = 'none';
    this.alertBox.style.zIndex = 100;
...
Thingiview.prototype.onRendererMouseMove = function (event)
{
    ...
    if (this.mouseDown)
    {
        ...
        this.xrot = this.targetXRotationOnMouseDown + (mouseX - mouseXOnMouseDown) * 0.02;

        this.mouseY = event.clientY - this.windowHalfY;
        // this.targetYRotation = this.targetYRotationOnMouseDown + (mouseY - mouseYOnMouseDown) * 0.02;
        this.yrot = this.targetYRotationOnMouseDown + (mouseY - mouseYOnMouseDown) * 0.02;

        this.targetXRotation = this.xrot;
        this.targetYRotation = this.yrot;
    }
};
...
Thingiview.prototype.onRendererTouchMove = function (event)
{
    if (event.touches.length == 1)
    {
        ...
        this.targetXRotation = this.targetXRotationOnMouseDown + (this.mouseX - this.mouseXOnMouseDown) * 0.05;

        this.mouseY = event.touches[0].pageY - this.windowHalfY;
        this.targetYRotation = this.targetYRotationOnMouseDown + (this.mouseY - this.mouseYOnMouseDown) * 0.05;
    }
};
...
Thingiview.prototype.setCameraView = function (dir)
{
    ...
    this.mouseX = this.targetXRotation;
    this.mouseXOnMouseDown = this.targetXRotation;

    this.mouseY = this.targetYRotation;
    this.mouseYOnMouseDown = this.targetYRotation;
...
Thingiview.prototype.centerCamera = function ()
{
    if (this.geometry)
    {
        ...
        var distance = this.geometry.boundingSphere.radius / Math.sin((this.camera.fov / 2) * (Math.PI / 180));
